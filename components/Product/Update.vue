...
onMounted(async () => {
    pending.value = true
    try {
        product.value = await getProductByID({ product_id: props.product_id });
    } catch (error) {
        ...
        <v-card-text>
            <v-row>
                <v-col cols="12" md="3">
                    ...
                    <v-text-field v-model="product.product_name" :error-messages="errors.product_name"
                        variant="outlined" density="compact" :placeholder="$t('product.name')" />
                </v-col>
                <v-col cols="12" md="3">
