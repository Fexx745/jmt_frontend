...
const { updateManagerBy, getManagerByID } = useManager();
...
onMounted(async () => {
    pending.value = true
    try {
        manager.value = await getManagerByID({ manager_id: props.manager_id });
    } catch (error) {
...
const onSubmit = async () => {
    ...
    try {
        await updateManagerBy(manager.value);
        manager.value = {
            manager_id: '',
            manager_name: ''
        };
        ...
        <v-card-text>
            <v-row>
                <v-col cols="12" md="12">
                    ...
                    <v-text-field v-model="manager.manager_name" :error-messages="errors.manager_name"
                        variant="outlined" density="compact" :placeholder="$t('manager.name')" />
                </v-col>
