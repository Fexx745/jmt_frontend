...
const { updateOrderBy, getOrderByID } = useOrder();
...
onMounted(async () => {
    pending.value = true
    try {
        order.value = await getOrderByID({ order_id: props.order_id });
    } catch (error) {
...
const onSubmit = async () => {
    ...
    try {
        await updateOrderBy(order.value);
        order.value = {
            order_id: '',
            order_name: '',
            order_price: 0,
        ...
        <v-card-text>
            <v-row>
                ...
                <v-col cols="12" md="12" style="margin-top: -30px;">
                    ...
                    <v-textarea v-model="order.order_detail" :placeholder="t('order.detail')" rows="3"
